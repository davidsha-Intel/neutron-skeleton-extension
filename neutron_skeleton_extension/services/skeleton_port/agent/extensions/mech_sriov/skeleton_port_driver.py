...
class SkeletonPortSriovDriver(skeleton_port_driver_base.SkeletonPortAgentDriver):

    def initailize(self):
        LOG.debug("\n\nInitializing Blank L2 SR-IOV driver\n\n")
        self.known_ports = []

    def port_created(self, port):
        p_id = port['port_id']
        LOG.debug("\n\nSR-IOV port_created called on port %s\n\n" % p_id)

    def port_updated(self, port):
        p_id = port['port_id']
        LOG.debug("\n\nSR-IOV port_updated called on port %s\n\n" % p_id)

    def port_deleted(self, port):
        p_id = port['port_id']
        LOG.debug("\n\nSR-IOV port_deleted called on port %s\n\n" % p_id)
